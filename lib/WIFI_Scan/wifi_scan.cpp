...
void wifi_scan_loop()
{
  ...
  if (millis() - prev > 1000)
  {
    ...
    lv_bar_set_value(bar1, temperature, LV_ANIM_ON);
    lv_bar_set_value(bar2, humidity, LV_ANIM_ON);
    lv_bar_set_value(bar3, suhu_air, LV_ANIM_ON);
    lv_bar_set_value(bar4, tds, LV_ANIM_ON);
    static char text1[20];

    sprintf(text1, "SUHU AIR : %.2f\u00b0c", temperature);
    lv_label_set_text(label1, text1);
    sprintf(text1, "pH : %.2fpH", humidity);
    lv_label_set_text(label2, text1);
    sprintf(text1, "SUHU : %.2f\u00b0c", temperature);
    lv_label_set_text(label3, text1);
    sprintf(text1, "TDS : %dppm", tds);
    lv_label_set_text(label4, text1);
    prev = millis();
  }
}
...
void home(void)
{
    ...
    lv_theme_t *theme = lv_theme_default_init(dispp, lv_palette_main(LV_PALETTE_BLUE), lv_palette_main(LV_PALETTE_RED), false, &lv_font_montserrat_12);
    lv_disp_set_theme(dispp, theme);

    int unarea = 0;//左侧不能触摸宽度
    screen = lv_obj_create(lv_scr_act());//null页面无显示
    lv_obj_clear_flag(screen, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_width(screen, 320);
    lv_obj_set_height(screen, 320);
    lv_obj_set_pos(screen,170,0);
    lv_obj_set_style_border_width(screen, 0, LV_PART_MAIN);
    // //橙色 不能触摸
    // lv_obj_t *screen2 = lv_obj_create(lv_scr_act());
    // lv_obj_clear_flag(screen2, LV_OBJ_FLAG_SCROLLABLE);
    // lv_obj_set_width(screen2, 160);
    // lv_obj_set_height(screen2, 320);
    // lv_obj_set_pos(screen2,0,0);
    // lv_obj_set_style_border_width(screen2, 0, LV_PART_MAIN);

    // lv_obj_t *icon_fill = lv_obj_create(screen2);
    // lv_obj_set_size(icon_fill, 160, 320);
    // lv_obj_set_pos(icon_fill, 0, 0);
    // lv_obj_set_style_radius(icon_fill, 5, LV_PART_MAIN);
    // lv_obj_set_style_bg_color(icon_fill, lv_color_hex(0xFFA500), LV_PART_MAIN);
    // lv_obj_set_style_border_width(icon_fill, 0, LV_PART_MAIN);
    // lv_obj_clear_flag(icon_fill, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_SCROLLABLE);
    // lv_obj_set_style_pad_all(icon_fill, 0, LV_PART_MAIN);

    //时钟
    lv_obj_t *clock_label = lv_label_create(screen);
    lv_obj_set_size(clock_label, LV_SIZE_CONTENT, LV_SIZE_CONTENT);
    lv_obj_set_pos(clock_label, 22+unarea, 9 - 5);
    ...
    lv_obj_set_pos(black, 10+unarea, 123 - 13);
    ...
    lv_obj_set_size(bigBlue, 200, 160);//大小需要调整
    lv_obj_set_pos(bigBlue, 93+unarea, 40 - 13);
    ...
    lv_obj_set_pos(fan_switch, 93+unarea, 197);
    ...
    lv_obj_set_pos(water_switch, 130+unarea, 197);
    ...
    lv_obj_set_pos(icon_mix, 170+unarea, 197);
    ...
    static lv_style_t style_indic;//内部指示
...
{
    //目录页
    menu_screen = lv_obj_create(NULL);//如果是screen会不显示home页 如果是lv_scr_act()会覆盖home的一部分

    lv_obj_clear_flag(menu_screen, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_width(menu_screen, 320);
    lv_obj_set_height(menu_screen, 320);
    ...
    lv_menu_set_mode_root_back_btn(menu, LV_MENU_ROOT_BACK_BTN_ENABLED);//根目录下是否显示返回按钮
    lv_obj_add_event_cb(menu, back_event_handler, LV_EVENT_CLICKED, menu);//根目录返回按键 
    lv_obj_set_size(menu, 320, 320);
    lv_obj_set_pos(menu,170,0);
  ...
  create_slider(section, LV_SYMBOL_SETTINGS, "Brightness", 0, 150, 100);
    ...
    //About页面下的 第一个目录子子页面 sub_software_info_page
    ...
    //About页面下的 第一个目录子子页面 sub_legal_info_page
  ...
  cont = create_text(section, LV_SYMBOL_AUDIO, "Brightness", LV_MENU_ITEM_BUILDER_VARIANT_1);  
  lv_menu_set_load_page_event(menu, cont, sub_display_page);

  create_text(root_page, NULL, "Others", LV_MENU_ITEM_BUILDER_VARIANT_1);
  section = lv_menu_section_create(root_page);
  cont = create_text(section, LV_SYMBOL_DRIVE, "About", LV_MENU_ITEM_BUILDER_VARIANT_1);
...
{
  xTaskCreate(beginWIFITask,//任务函数
              "beginWIFITask",//任务名称
              2048,//任务栈大小
              NULL,//任务参数
              1,//任务优先级
              &ntConnectTaskHandler//任务句柄
              );
}
...
static void beginWIFITask(void *pvParameters)
{
  ...
  while (WiFi.status() != WL_CONNECTED && (millis() - startingTime) < (20 * 1000))
  {
    vTaskDelay(2000);
  }
